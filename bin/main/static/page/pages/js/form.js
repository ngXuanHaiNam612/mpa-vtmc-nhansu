    ...
    try{
        ...
        for(let i = 0 ; i < listPhongBan.length ; i++){
            tbodyString += `<tr>`+
                ...
                `<td>${listPhongBan[i].maTruongPhong}</td>`+
                `</tr>`;
        }
...
createSelectBoxPhongBan = async () =>{
    let selectString = `<select id="cbxPhongBan" class="form-select bg-body" aria-label="Default select example">`;
    ...
    selectString += `</select>`;
    let tSelect = document.getElementById("cbxPhongBan");
